...
const buttyesFun= () => {
    ...
    new_prompt.innerText = "Hemos recibido la información de su donación. Muchas gracias." ;
    div_conf.appendChild(new_prompt);
    let button = document.createElement("button");
    button.type = "button";
    button.innerText = "Ir al inicio";
    button.addEventListener("click",butt_inicio);
    div_conf.appendChild(button);
}
const buttnoFun = () => {
    let div_conf = document.getElementById("confirm");
    div_conf.remove();
...
const validarDonacion = () => {
    const validadorRegion = (region) => region && regionesList.includes(region);
    const validadorComuna = (region,comuna) => {
        let res = false;
        const indexR = regionesList.indexOf(region);
        if (validadorRegion(region) && comuna){
            for (i in data["regiones"][indexR]["comunas"]){
                if (comuna == data["regiones"][indexR]["comunas"][i]["nombre"]){
                    res = true;
                    break;
                }
            }
    ...
    const validadorImg = (img1,img2,img3) => img1 || img2 || img3;
    ...
    let fechaDisponibilidadInput = document.getElementById("fecha-disponibilidad");
    let descripcionInput = document.getElementById("descripcion");
    let condicionesInput = document.getElementById("condiciones");
    let fotosInput = document.getElementById("fotos-input")
    let foto1Input = document.getElementById("foto-1")
    let foto2Input = document.getElementById("foto-2")
    let foto3Input = document.getElementById("foto-3")
    ...
    if (!validadorComuna(regionInput.value, comunaInput.value)){
        msg += "¡Comuna invalida!\n";
            comunaInput.style.borderColor = "red"; 
    } else {
            comunaInput.style.borderColor = "";
    }
    if (!validadorText(calleNumeroInput.value,80)){
        msg += "¡Dirección invalida!\n";
            calleNumeroInput.style.borderColor = "red"; 
    } else {
    ...
    if (!validadorFecha(fechaDisponibilidadInput.value)){
        msg += "¡Fecha de Disponibilidad invalida!\n";
            fechaDisponibilidadInput.style.borderColor = "red";
    } else {
            fechaDisponibilidadInput.style.borderColor = "";
    }
    if (!validadorImg(foto1Input.value,foto2Input.value,foto3Input.value)){
        msg += "¡Imágenes inválidas!\n";
            fotosInput.style.Color = "red";
    } else {
            fotosInput.style.Color = "";
    }
    ...
    if (msg === ""){
        msg = "¡Has completado tu donación!";
        let form_don = document.getElementById("form-donacion");
        let question = document.createElement("div");
        question.id = "confirm";
        question.className = "grupo-input";
        let prompt = document.createElement("h3");
        prompt.innerText= "¿Confirma que desea agregar esta donación?";
        ...
        question.appendChild(butt_yes);
        question.appendChild(butt_no);
        butt_yes.addEventListener("click", buttyesFun);
        butt_no.addEventListener("click", buttnoFun);
        form_don.appendChild(question);
    }
    else {
        alert(msg);
    }
...
let regionesTable = document.getElementById("regiones")
let comunasTable = document.getElementById("comunas")
let regionInput = document.getElementById("region");
let comunaInput = document.getElementById("comuna");

for (let x in regionesList){
    const reg = document.createElement("option");
    reg.innerText = regionesList[x];
    regionesTable.append(reg);
}

const comunasSelect = () => {
    comunaInput.value = "";
    comunasTable.innerHTML = "";
    if (regionesList.includes(regionInput.value)){
        const index = regionesList.indexOf(regionInput.value)
        for (i in data["regiones"][index]["comunas"]){
            const reg = document.createElement("option");
            reg.innerText = data["regiones"][index]["comunas"][i]["nombre"];
            comunasTable.append(reg);
        }
    }
}
